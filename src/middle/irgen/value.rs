...
/// Collapse a possibly flattened array to nested
///
/// # Panics
/// Please make sure `arr` is non-empty.
pub fn collapse_array(arr: &mut VecDeque<Value>, ty: &ValueType) -> Result<Value> {
    if let ValueType::Array(element_ty, len) = ty {
        ...
        for _ in 0..*len {
            ...
            if let Value::Array(arr) = first_item {
...
mod tests {
    ...
    use crate::middle::irgen::value::{collapse_array, Value};

    #[test]
    fn test_collapse_flattened_array() {
        let arr = vec![
            Value::ReadOnly(Constant::Int(1).into()),
            Value::ReadOnly(Constant::Int(2).into()),
            Value::ReadOnly(Constant::Int(3).into()),
            Value::ReadOnly(Constant::Int(4).into()),
        ];
        let mut vec_deque = VecDeque::from(arr);
        let ty = ValueType::Array(ValueType::Array(ValueType::Int.into(), 2).into(), 2);
        let res = collapse_array(&mut vec_deque, &ty).unwrap();
        assert_eq!(
            res,
            Value::Array(vec![
                Value::Array(vec![
    ...
    #[test]
    fn test_collapse_mixed_array_2() {
        let arr = vec![
            Value::Array(vec![
                Value::ReadOnly(Constant::Int(1).into()),
                Value::ReadOnly(Constant::Int(2).into()),
    ...
    #[test]
    fn test_collapse_fractured_array() {
        let arr = vec![
            Value::Array(vec![Value::ReadOnly(Constant::Int(1).into())]),
            Value::ReadOnly(Constant::Int(3).into()),
        ];
        let mut vec_deque = VecDeque::from(arr);
        let ty = ValueType::Array(ValueType::Array(ValueType::Int.into(), 2).into(), 2);
        ...
        assert_eq!(
            ...
            Value::Array(vec![
                Value::Array(vec![Value::ReadOnly(Constant::Int(1).into()),]),
                Value::Array(vec![Value::ReadOnly(Constant::Int(3).into()),]),
            ])
        );
    }
}
