...
impl Block {
    pub fn live_use_regs2(&self) -> RegSet {
        let mut live_use = RegSet::new();
        for inst in self.insts().iter().rev() {
            for r in inst.defs() {
                live_use.remove(r);
            }
    ...
    pub fn live_def_regs2(&self) -> RegSet {
        let mut live_def = RegSet::new();
        for inst in self.insts().iter().rev() {
            for r in inst.defs() {
                live_def.insert(r);
            }
...
impl Func {
    /// FIXME test needed
    /// compute the live use set of regs of each basic block
    pub fn reg_live_use2(f: &Func) -> HashMap<String, RegSet> {
        f.iter_bbs()
            .map(|bb| (bb.label().to_string(), bb.live_use_regs2()))
            .collect()
    }

    /// FIXME test needed
    pub fn reg_live_def2(f: &Func) -> HashMap<String, RegSet> {
        f.iter_bbs()
            .map(|bb| (bb.label().to_string(), bb.live_def_regs2()))
            .collect()
    }

    /// FIXME test needed
    pub fn reg_lives2(f: &Func) -> Result<RegLives2> {
        let (ins, outs) = Func::in_out_bbs(f)?;
        let mut live_ins: HashMap<String, RegSet> = HashMap::new();
        let mut live_outs: HashMap<String, RegSet> = HashMap::new();
        ...
        // consider live_use
        let live_use = Func::reg_live_use2(f);
        let live_def = Func::reg_live_def2(f);
        ...
        while changed {
            ...
            // new_live_in = old_live_in U ( SUM(live_in[out[bb]]) - live_def[bb] ) U new_live_out
            for bb in bb_iter.clone() {
                let mut new_live_in = live_ins.get(bb.label()).cloned().unwrap_or(RegSet::new());

                for in_bb in ins.ins(bb) {
                    if let Some(out) = live_outs.get(in_bb.label()) {
                        new_live_in.merge(out);
                    }
                }
                new_live_in.retain(|r| !live_def[bb.label()].contains(r));

                let mut new_live_out = live_outs.get(bb.label()).cloned().unwrap_or(RegSet::new());
                for out_bb in outs.outs(bb) {
                    if let Some(in_) = live_ins.get(out_bb.label()) {
                        new_live_out.merge(in_);
                    }
                }

                for r in new_live_out.iter() {
                    if !live_def[bb.label()].contains(&r) {
    ...
    pub fn reg_interfere_graph2(f: &Func) -> Result<HashMap<Reg, RegSet>> {
        ...
        for bb in f.iter_bbs() {
            let mut alive_regs: RegSet = reg_lives.live_outs(bb)?.clone();
...
pub struct RegLives2 {
    live_ins: HashMap<String, RegSet>,
    live_outs: HashMap<String, RegSet>,
}
...
impl RegLives2 {
    pub fn live_ins(&self, bb: &Block) -> Result<&RegSet> {
        self.live_ins
            .get(bb.label())
            .ok_or_else(|| anyhow::anyhow!("live_ins not found for block {}", bb.label()))
    }

    pub fn live_outs(&self, bb: &Block) -> Result<&RegSet> {
        self.live_outs
            .get(bb.label())
            .ok_or_else(|| anyhow::anyhow!("live_outs not found for block {}", bb.label()))
    }
}
