...
pub fn gen_from_clang(program: &clang_frontend::Program) -> Result<Program, BackendError> {
    ...
    for f in &llvm.functions {
        // dbg!(&f);
        let args: Vec<String> = f.parameters
            .iter()
        ...
        Ok(match operand {
            llvm_ir::Operand::LocalOperand { name, ty } => {
                let ss = stack_slots
                    .get(name)
                    .ok_or(
                        BackendError::GenFromLlvmError(format!("stack slot {} not found", name))
                    )?;
...
fn build_store_inst(
    ...
    match val {
        ...
        Operand::Fmm(_) => {
            return Err(
                BackendError::GenFromLlvmError("store instruction with float value".to_string())
            );
        }
        _ => (),
    }
